...
object GuildDatabase {
    ...
    val fireSkills = listOf(
        GuildSkillRequirement(Skill("Ignite", "A basic fire spell.", 15, 0, 10, "Attack", 1.2, elementType = ElementType.Fire), 1, 1000),
        GuildSkillRequirement(Skill("Fireball", "A classic projectile.", 25, 1, 15, "Attack", 1.5, elementType = ElementType.Fire), 2, 3000),
        GuildSkillRequirement(Skill("Flame Pillar", "Erupting fire from beneath.", 40, 2, 25, "Attack", 2.0, elementType = ElementType.Fire), 3, 7500),
        GuildSkillRequirement(Skill("Heat Wave", "A wide area burn.", 50, 3, 35, "Attack", 1.8, effectType = "Burn", elementType = ElementType.Fire, statusEffect = StatusEffect.Burn), 4, 15000),
        GuildSkillRequirement(Skill("Inferno", "A devastating fire storm.", 75, 5, 50, "Attack", 3.0, elementType = ElementType.Fire), 5, 30000),
        GuildSkillRequirement(Skill("Blazing Soul", "Boosts fire damage significantly.", 60, 4, 45, "Buff", 1.5, effectType = "Buff", elementType = ElementType.Fire), 6, 50000),
        GuildSkillRequirement(Skill("Dragon Breath", "Mimics the dragon's fire.", 100, 6, 60, "Attack", 4.0, elementType = ElementType.Fire), 7, 75000),
        GuildSkillRequirement(Skill("Solar Flare", "Blinds and burns the enemy.", 120, 8, 70, "Attack", 3.5, effectType = "Stun", elementType = ElementType.Fire, statusEffect = StatusEffect.Stun), 8, 100000),
        GuildSkillRequirement(Skill("Phoenix Rebirth", "Heals and buffs fire power.", 150, 10, 80, "Support", 2.0, effectType = "Heal", elementType = ElementType.Fire), 9, 250000),
        GuildSkillRequirement(Skill("Supernova", "The ultimate fire magic.", 250, 12, 100, "Attack", 6.0, effectType = "LimitBreak", elementType = ElementType.Fire), 10, 500000)
    ).take(15) // User requested 15, I'll add 10 for now and placeholder the rest if needed

    val waterSkills = listOf(
        GuildSkillRequirement(Skill("Water Bolt", "A concentrated stream of water.", 12, 0, 10, "Attack", 1.1, elementType = ElementType.Ice), 1, 1000),
        GuildSkillRequirement(Skill("Aqua Shield", "Reduces incoming damage.", 30, 2, 15, "Buff", 0.0, effectType = "Buff", elementType = ElementType.Ice), 2, 3000),
        GuildSkillRequirement(Skill("Ice Spike", "Piercing ice from the ground.", 35, 1, 25, "Attack", 1.7, elementType = ElementType.Ice), 3, 7500),
        GuildSkillRequirement(Skill("Healing Rain", "Gentle water that restores HP.", 45, 3, 30, "Support", 1.5, effectType = "Heal", elementType = ElementType.Ice), 4, 15000),
        GuildSkillRequirement(Skill("Blizzard", "Freezing winds and ice.", 80, 5, 50, "Attack", 2.5, effectType = "Freeze", elementType = ElementType.Ice), 5, 30000),
        GuildSkillRequirement(Skill("Tidal Wave", "A massive wave of force.", 100, 4, 60, "Attack", 3.5, elementType = ElementType.Ice), 6, 50000),
        GuildSkillRequirement(Skill("Absolute Zero", "Stops the enemy in their tracks.", 150, 8, 75, "Attack", 4.0, effectType = "Stun", elementType = ElementType.Ice, statusEffect = StatusEffect.Stun), 7, 100000),
        GuildSkillRequirement(Skill("Mist Cover", "Increases dodge chance.", 60, 4, 40, "Buff", 0.0, effectType = "Buff", elementType = ElementType.Ice), 8, 150000),
        GuildSkillRequirement(Skill("Whirlpool", "Drains enemy mana.", 120, 6, 70, "Attack", 2.0, elementType = ElementType.Ice), 9, 300000),
        GuildSkillRequirement(Skill("Oceanic Wrath", "The power of the deep sea.", 300, 15, 100, "Attack", 7.0, effectType = "LimitBreak", elementType = ElementType.Ice), 10, 750000)
    )

    val windSkills = listOf(
        GuildSkillRequirement(Skill("Wind Slash", "A sharp gust of air.", 10, 0, 10, "Attack", 1.0, elementType = ElementType.Physical), 1, 1000),
        GuildSkillRequirement(Skill("Swift Step", "Increases agility.", 20, 2, 15, "Buff", 0.0, effectType = "Buff", elementType = ElementType.Physical), 2, 3000),
        GuildSkillRequirement(Skill("Tornado", "A spinning vortex of wind.", 50, 4, 30, "Attack", 2.2, effectType = "Stun", elementType = ElementType.Physical, statusEffect = StatusEffect.Stun), 3, 8000),
        GuildSkillRequirement(Skill("Air Blade", "Multiple strikes with air.", 40, 2, 25, "Attack", 1.8, effectType = "MultiHit", elementType = ElementType.Physical), 4, 16000),
        GuildSkillRequirement(Skill("Vacuum Burst", "Sucks in the enemy and explodes.", 70, 5, 45, "Attack", 2.8, elementType = ElementType.Physical), 5, 35000),
        GuildSkillRequirement(Skill("Eye of the Storm", "Ultimate focus and speed.", 100, 6, 60, "Buff", 2.0, effectType = "CritBuff", elementType = ElementType.Physical), 6, 60000),
        GuildSkillRequirement(Skill("Cyclone Strike", "Heavy wind damage.", 130, 8, 75, "Attack", 4.5, elementType = ElementType.Physical), 7, 120000),
        GuildSkillRequirement(Skill("Feather Weight", "Ignore weight, move first.", 50, 4, 40, "Buff", 0.0, effectType = "Buff", elementType = ElementType.Physical), 8, 200000),
        GuildSkillRequirement(Skill("Thunderstorm", "Wind and lightning combined.", 180, 10, 85, "Attack", 5.0, elementType = ElementType.Lightning), 9, 400000),
        GuildSkillRequirement(Skill("God Wind", "The breath of the wind god.", 400, 20, 100, "Attack", 10.0, effectType = "LimitBreak", elementType = ElementType.Physical), 10, 1000000)
    )

    fun getGuildSkills(guildName: String): List<GuildSkillRequirement> {
