...
import com.example.lumeria.managers.BattleOrchestrator
import com.example.lumeria.managers.EconomyManager
import com.example.lumeria.models.Screen
import com.example.lumeria.models.StoryArc
import com.example.lumeria.utils.AchievementManager
...
class MainViewModel(private val repository: PlayerDataRepository) : ViewModel() {
    ...
    val settings = repository.settingsFlow.stateIn(
        scope = viewModelScope,
        started = SharingStarted.WhileSubscribed(5000),
        initialValue = com.example.lumeria.data.GameSettings()
    )
    ...
    fun updatePlayer(newData: PlayerData) {
        ...
        val finalData = if (newlyUnlocked.isNotEmpty()) {
            newData.copy(unlockedAchievements = (newData.unlockedAchievements + newlyUnlocked).distinct())
        } else newData
        
        viewModelScope.launch {
            repository.savePlayerData(finalData, activeSlot)
        }
    }

    fun updateSettings(newSettings: com.example.lumeria.data.GameSettings) {
        viewModelScope.launch {
    ...
    fun onBattleLeave(newData: PlayerData, victoryProcessed: Boolean) {
        ...
        if ((currentArc != null) && victoryProcessed) {
            ...
            if (currentEventIndex >= currentArc.events.size) {
                processArcCompletion(finalData, currentArc)
            } else {
            ...
            val nextFloor = finalData.towerFloor + 1
            val updated = finalData.copy(
                towerFloor = nextFloor,
                towerHighestFloor = maxOf(finalData.towerHighestFloor, finalData.towerFloor)
            )
            updatePlayer(updated)
            popBackstack() // Return to Tower
        } else if (currentExam != null && victoryProcessed) {
            val nextLvl = finalData.guildLevel + 1
            val updated = finalData.copy(
                guildLevel = nextLvl,
                guildXp = 0
            )
            ...
            if (victoryProcessed && !finalData.towerUnlockedMessageSeen && TowerDatabase.isTowerUnlocked(finalData.defeatedBosses)) {
        ...
        if (currentData.activeBuffs.isNotEmpty()) {
            val updatedBuffs = currentData.activeBuffs.asSequence()
                .map { it.copy(battlesRemaining = it.battlesRemaining - 1) }
                .filter { it.battlesRemaining > 0 }
                .toList()
            updatePlayer(currentData.copy(activeBuffs = updatedBuffs))
        }
    }
    ...
    private fun processArcCompletion(newData: PlayerData, arc: StoryArc) {
        ...
        val newQuests = GameDatabase.allQuests.filter {
        ...
        val stats = com.example.lumeria.models.XiousStats(newData.level, newData.xp).gainXp(arc.rewardXp)
    ...
    fun onArenaBattleStart(opponent: com.example.lumeria.database.ArenaOpponent) {
    ...
    fun onStartGuildExam(exam: com.example.lumeria.database.GuildDatabase.GuildExam) {
        battleOrchestrator.startGuildExam(exam, playerData.value)
    }

    fun onFishCaught(fish: com.example.lumeria.database.Fish) {
        val current = playerData.value
    ...
    fun onEnterCode(code: String, deviceId: String): String {
        return when (code.trim()) {
            "Materger" -> {
                ...
                val current = playerData.value
                val allArenaNames = com.example.lumeria.database.ArenaEnemyDatabase.opponents.map { it.enemy.name }
